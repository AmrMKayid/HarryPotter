...
public class M1PrivateTest {
	...
	public void testConstructorThirdTaskInitialization() throws Exception {
		...
		ArrayList<Champion> champs = (ArrayList<Champion>) myObjMethod.invoke(myObj);
		assertTrue("The constructor of the ThirdTask class should not randomly shuffle the champions list",champs.get(0).equals(g) && champs.get(1).equals(h)
	...
	public void testSecondTaskMapChampionLocation() throws Exception {
		for (int i = 0; i < 100; i++) {
			...
			assertTrue("The first champion position is incorrect.\n"
				...
				for (int j = 0; j < currentMap[i].length; j++) {
					if (generatedMap[i][j] == 0)
						assertTrue(
								...
								currentMap[i][j] instanceof EmptyCell || currentMap[i][j] instanceof CollectibleCell);
